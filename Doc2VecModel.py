...
class Doc2VecModel:
    def __init__(self, seed=1, num_features = 20, min_word_count = 2, context_size = 6):
        ...
        self.outputmodelfilename = GV.basedirectoy + "pubmedd2vmodel_F_" + str(self.num_features) \
                                   + "_C_" + str(self.context_size) + ".model"
    ...
    def CreateTaggedDocuments(self, tokenizeddocs, ids):
        ...
        if fo.exists(GV.taggedDocumentFile):
            ...
            taggeddocuments = [gensim.models.doc2vec.TaggedDocument(s, [ids[i]]) for i, s in tqdm(enumerate(tokenizeddocs))]
    ...
    def Model(self, docs, ids):
        ...
        self.doc2vec_model.train(docs, total_examples=self.doc2vec_model.corpus_count, epochs=self.doc2vec_model.iter)
    ...
    def LoadModel(self, filename=None):
        if filename is None:
            self.doc2vec_model = gensim.models.doc2vec.Doc2Vec.load(self.outputmodelfilename)
        else:
    ...
    def SaveSimilarDocuments(self, pubmedarticlelists, similardocfilename):
        ...
        for idx, pmid in tqdm(enumerate(pmids)):
            ...
            similardocdict[pmid] = self.doc2vec_model.docvecs.most_similar(pmid, topn=23752)
            similardocdict[pmid].insert(0, (pmid, '1.0'))

            #TODO New code
            if idx % 1000 == 0 or idx == 23753:
                with open('./saveddata/simdictpmid.pkl', mode='a+b') as f:  # appending, not writing
        ...
        for idx, pmid in tqdm(enumerate(pmids)):
            ...
            similararticlesdict[pmid] = {'Title': doctitle, 'Similar': similartitlescorelist}
    ...
    def Visualize(self, pmid_choice):
        ...
        plt.legend()
        plt.savefig('newtempfile_' + str(self.num_features) + "_" + str(self.context_size) + "_" + '_new.png')
        plt.show()
